...
upper.bound <- round(length(sorted.thres) * 0.9, 0)

taugrid <- sorted.thres[sorted.thres >= 4 & sorted.thres <= upper.bound]
#quantiles <- quantile(thdvar, probs = c(0.20, 0.80))
...
RZdf <- RZdf %>%
  mutate(
    high_unemploy = ifelse(unemp > best_tau , 1, 0),
    lag_high_unemploy = lag(high_unemploy)
  ) %>%
  na.omit()
...
nl_g<-HDLP(x=dc$newsy, y=dc$g, q=cbind(dc$y,dc$taxy), state_variables=dc$lag_high_unemploy, lags=lags, hmax=hmax)
nl_y<-HDLP(x=dc$newsy, y=dc$y, q=cbind(dc$g,dc$taxy), state_variables=dc$lag_high_unemploy, lags=lags, hmax=hmax)
...
p2<-ggplot(data=g_df, aes(x=Horizon)) +
  ...
  scale_colour_manual(values = c("black",col1, col2), labels=c("Linear model","Low unemployment", "High unemployment"), breaks = c("linear","high", "low"),
...
p6<-ggplot(data=y_df, aes(x=Horizon)) +
  ...
  scale_colour_manual(values = c(col1, col2), labels=c("Low unemployment", "High unemployment"), breaks = c("high", "low"),
                      guide = guide_legend(direction = "horizontal", title.position = "top",
                                           label.position="bottom", label.hjust = 0.5, label.vjust = 0.5,
                                           label.theme = element_text(angle = 90)))+
  geom_ribbon_pattern(aes(ymin = lb_hu, ymax = ub_hu), fill = col1, alpha=0.1,
